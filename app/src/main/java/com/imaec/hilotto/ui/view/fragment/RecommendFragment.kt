...
class RecommendFragment :
    BaseFragment<FragmentRecommendBinding>(R.layout.fragment_recommend),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.apply {
            ...
            vm = this@RecommendFragment.viewModel
            switchCondition1.isChecked = SharedPreferenceUtil.getBool(
                requireContext(),
                SharedPreferenceUtil.KEY.PREF_RECOMMEND_CONDITION_SUM,
                true
            )
            switchCondition1.setOnCheckedChangeListener(this@RecommendFragment)
            switchCondition2.isChecked = SharedPreferenceUtil.getBool(
                requireContext(),
                SharedPreferenceUtil.KEY.PREF_RECOMMEND_CONDITION_PICK,
                true
            )
            switchCondition2.setOnCheckedChangeListener(this@RecommendFragment)
            switchCondition3.isChecked = SharedPreferenceUtil.getBool(
                requireContext(),
                SharedPreferenceUtil.KEY.PREF_RECOMMEND_CONDITION_ODD_EVEN,
                true
            )
            switchCondition3.setOnCheckedChangeListener(this@RecommendFragment)
            switchConditionAll.isChecked = SharedPreferenceUtil.getBool(
                requireContext(),
                SharedPreferenceUtil.KEY.PREF_RECOMMEND_CONDITION_ALL,
                true
            )
                ...
                adapter = NumberAdapter().apply {
                    addOnClickListener { number ->
                        if (number is String) {
                            CommonDialog(
                                requireContext(),
                                requireContext().getString(R.string.msg_remove_number)
                            ).apply {
                                setOnOkClickListener(
                                    View.OnClickListener {
                                        this@RecommendFragment.viewModel.removeNotIncludeNumber(
                                            number
                                        )
                                        dismiss()
                                    }
    ...
    override fun onCheckedChanged(cp: CompoundButton, b: Boolean) {
        when (cp.id) {
            R.id.switch_condition1 -> {
                ...
                setCheck(
                    b && binding.switchCondition2.isChecked &&
                        binding.switchCondition3.isChecked,
                    b, cp as SwitchMaterial
                )
            }
            R.id.switch_condition2 -> {
                ...
                setCheck(
                    b && binding.switchCondition1.isChecked &&
                        binding.switchCondition3.isChecked,
                    b, cp as SwitchMaterial
                )
            }
        ...
        when (view.id) {
            ...
            R.id.image_not_included_add -> {
                if (viewModel.listNotIncludeNumber.value!!.size == 6) {
                    Toast.makeText(
                        context,
                        context?.getString(R.string.msg_not_include_number_max),
                        Toast.LENGTH_SHORT
                    ).show()
                    return
                }
            ...
            R.id.text_include_confirm -> {
                if (pickerFlag == 0) {
                    if (viewModel.checkNotIncludeNumber("$pickedNumber")) setNumber()
                    else Toast.makeText(
                        context,
                        R.string.msg_exist_not_include_number,
                        Toast.LENGTH_SHORT
                    ).show()
                } else {
                    if (viewModel.checkIncludeNumber("$pickedNumber")) {
                        viewModel.addNotIncludeNumber("$pickedNumber")
                    } else {
                        Toast.makeText(
                            context,
                            R.string.msg_exist_include_number,
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
            ...
            R.id.text_save -> {
                if (getEmptyCount() > 0) {
                    Toast.makeText(context, R.string.msg_number_not_created, Toast.LENGTH_SHORT)
                        .show()
                    return
                }
                viewModel.apply {
                        ...
                        saveNumber(
                            ...
                            showAd(R.string.ad_id_recommend_front, true) {
                                if (isSuccess) {
                                    Toast.makeText(
                                        context,
                                        R.string.msg_success_save_number,
                                        Toast.LENGTH_SHORT
                                    ).show()
                                    clearNumber()
                                } else Toast.makeText(
                                    context,
                                    R.string.msg_numbers_is_exist,
                                    Toast.LENGTH_SHORT
                                ).show()
                            }
                        }
