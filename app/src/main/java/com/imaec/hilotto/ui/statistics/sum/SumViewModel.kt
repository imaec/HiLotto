...
class SumViewModel @Inject constructor() : BaseViewModel() {
    ...
    private val _sumList = MutableLiveData<List<SumVo>>()
    val sumList: LiveData<List<SumVo>> get() = _sumList
    ...
    fun setSumList(lottoList: List<LottoVo>?, isInclude: Boolean = false) {
        lottoList ?: return

        val listTemp = mutableListOf<SumVo>()
    ...
    private fun setSum(list: List<SumVo>) {
        var sum = 0
        list.forEach { dto ->
            sum += dto.sum
